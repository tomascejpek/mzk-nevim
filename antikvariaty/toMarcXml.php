...
if (isset($opt['u'])) $time = $opt['u'];
else die("-u: Last update is not set (in hours)!\n");
...
$lastImport = date('U') - $time * 3600;
foreach ($xmlReader->record as $record) {
    if ($time > 0) {
        $lastUpdated = date('U', strtotime($record->datestamp));

        if ($lastUpdated < $lastImport) {
            continue;
        }
    }
